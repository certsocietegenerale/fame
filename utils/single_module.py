...
class Dispatcher:
    def __init__(self, interactive):
        ...
        for loader, name, ispkg in pkgutil.walk_packages(
            [MODULES_ROOT], prefix="fame.modules."
        ):
            if not ispkg:
                try:
        ...
        for named_config in module.named_configs:
            ...
            for setting in module.named_configs[named_config]["config"]:
                if self.interactive or "default" not in setting:
                    values[setting["name"]] = self.get_value_from_user(
                        setting, named_config
                    )
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "module", metavar="module", type=str, help="The name of the module to run."
    )
