...
from .config import ROLE_MAPPING, CLAIM_MAPPING
...
def authenticate(oidc_token):
    ...
    for elem in ["email", "name", "role"]:
        try:
            claim[elem] = parse(CLAIM_MAPPING[elem]).find(userinfo)[0].value
