    ...
    def repository_new(self):
        ...
        if request.method == "POST":
            ...
            if existing_repository:
                flash(
                    "There is already a repository with this name.".format(field),
                    "danger",
                )
                return validation_error()
            existing_repository = Repository.get(
                **{"branch": repository["branch"], "address": repository["address"]}
            )
            if existing_repository:
                flash(
                    "There is already a repository with this address/branch couple.".format(
                        field
                    ),
                    "danger",
                )
