...
class ModuleDispatcher(object):
    def reload(self):
        ...
        self._general = []
        self.options = {"str": {}, "text": {}, "integer": {}, "bool": {}}
        delete_perm_str = "anonymize (soft-delete)"
        if fame_config.get("files_removal_method", "") == "delete":
            delete_perm_str = "delete"
        ...
        self.permissions = OrderedDict(
            [
                ...
                (
                    "manage_modules",
                    "Allows for module operations (installation, configuration, etc.)",
                ),
                (
                    "manage_users",
                    "Allows for users management (creation, modification, etc.)",
                ),
                (
                    "configs",
                    'Grants access to the "Configs" section that tracks malware configurations.',
                ),
                (
                    "submit_iocs",
                    "Allows user to send observables to Threat Intelligence modules.",
                ),
                ("add_probable_name", "Allows user to set an object's probable name"),
                (
                    "see_logs",
                    "Allows user to access the log section of anlyses. This could reveal information on the underlying system.",
                ),
                (
                    "review",
                    "Allows user to review analyses. This is reserved to users analyzing submissions.",
                ),
                (
                    "delete",
                    f"Allows user to {delete_perm_str} files and associated analyses.",
                ),
            ]
        )
    ...
    def get_module(self, module_name):
        return (
            self.get_preloading_module(module_name)
            or self.get_processing_module(module_name)
            or None
        )
    ...
    def next_module(self, types_available, module_name, excluded_modules):
        ...
        if not module.info["acts_on"]:
            ...
            for acts_on in iterify(module.info["acts_on"]):
                ...
                return self._shortest_path_to_module(
                    types_available, module, excluded_modules
                )
    ...
    def next_preloading_module(self, selected_modules=[], excluded_modules=[]):
        ...
        for module in self.get_preloading_modules():
            if (
                ...
                if (
                    smallest_priority is None
                    or module_info["priority"] < smallest_priority
                ):
    ...
    def add_module(self, module):
        ...
        if m:
            ...
            try:
                ...
                print(
                    ("Could not initialize module '{0}': {1}".format(module["name"], e))
                )
            ...
            # Otherwise, add to general purpose modules
                ...
                if module["acts_on"]:
                    for source_type in iterify(module["acts_on"]):
                        self._add_trigger(
                            self._triggers, "_generated_file(%s)" % source_type, module
                        )
            ...
            if "option" in config and config["option"]:
                if config["name"] not in self.options[config["type"]]:
                    self.options[config["type"]][config["name"]] = {
                        ...
                        or (config["default"] if "default" in config else None),
                        "display": (
                            config["display"] if "display" in config else config["name"]
                        ),
                        "description": config["description"],
                        "modules": [],
                    }

                self.options[config["type"]][config["name"]]["modules"].append(
                    module["name"]
                )
            ...
            for source_type in iterify(module["acts_on"]):
                ...
                for generated_type in iterify(module["generates"]):
                    self._transforms[source_type].append(
                        {"type": generated_type, "module": module["name"]}
                    )
    ...
    def walk_modules(self, modules_dir, repository=None):
        ...
        for loader, name, ispkg in pkgutil.walk_packages(
            [modules_dir], prefix="fame.modules."
        ):
            if not ispkg:
                if repository is None or name.startswith(
                    "fame.modules.{}.".format(repository["name"])
                ):
                    try:
                        ...
                        error = (
                            "Could not import {}. The module was ignored.\n{}\n".format(
                                name, tb
                            )
                        )
                        if repository:
                ...
                for name, obj in self.walk_modules(modules_dir):
                    if obj.name and updated_named_config["name"] in obj.named_configs:
                        ...
                        if info["enabled"]:
                            print(
                                (
                                    "Disabling {} for incomplete named config {}".format(
    ...
    def _shortest_path_to_module(
        self, types_available, target_module, excluded_modules
    ):
        next_module = None
        path_length = None
    ...
    def _shortest_path_to_type(
        self, types_available, destination_type, excluded_modules
    ):
        next_module = None
        path_length = None

        for source_type in types_available:
            module, length = self._shortest_path(
                source_type, destination_type, excluded_modules
            )
            if path_length is None or length < path_length:
                path_length = length
                next_module = module
        ...
        if path_length == 1:
            ...
            direct_transform = self._get_direct_transform(
    ...
    def _shortest_path(
        self, source_type, destination_type, excluded_modules, excluded_types=None
    ):
        next_module = None
        path_length = None
        ...
        if source_type in self._transforms:
            for transform in self._transforms[source_type]:
                if transform["module"] not in excluded_modules:
                    length = self._path_length_from_module(
                        transform, destination_type, excluded_modules, excluded_types
                    )
                    if (length is not None) and (
                        path_length is None or length < path_length
                    ):
                        path_length = length
    ...
    def _path_length_from_module(
        self, transform, destination_type, excluded_modules, excluded_types
    ):
        if transform["type"] in excluded_types:
            ...
            module, length = self._shortest_path(
                transform["type"], destination_type, excluded_modules, excluded_types
            )
